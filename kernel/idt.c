...
void init_idt (void)
{
  ...
  set_task_int_gate (__system_call_entry, SYSCALL_VECTOR, D_DPL0, &system_call_tss, GDT_INT_START_INDEX + 32);


  /* set the IDTR register */
  load_idtr();
  
  DEBUG_MSG_NB ("IDT initialized ok. IDT @ ");
  printk_long ((long) idt);
  newline();
}
...
void set_task_int_gate (void (*handler)(void), int int_no, int dpl, tss_386_t  *tss, int  gdt_index)
{
  ...
  idt_ptr->selector = (gdt_index << 3) | dpl;
  idt_ptr->pad = 0;
